...
${rules}`;
  const text = await generateText(prompt, { temperature: 1.0, maxOutputTokens: 1000 });
...
${rules}`;
  const text = await generateText(prompt, { temperature: 1.0, maxOutputTokens: 900 });
...
${rules}`;
  const text = await generateText(prompt, { temperature: 1.0, maxOutputTokens: 800 });
  return { text: cleanScript(text), dj };
}
