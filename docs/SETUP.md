...
- **`kokoro synthesis timed out`** — synthesis started but never finished. If you see this on an up-to-date checkout, capture the script it was reading: a clip that takes over two minutes on CPU is unusual, and the timeout is in `SYNTH_TIMEOUT_MS` (`server/ai/kokoroVoice.js`).
