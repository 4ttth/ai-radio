...
- **Local voice is slow / `kokoro synthesis timed out`** — every successful segment logs its speed, e.g. `chars=283 ms=33755 rate=8.4ch/s`. Speech is roughly 15 ch/s, so anything under ~15 ch/s means synthesis is slower than real time, and a long news bulletin can take minutes. Things to try, measuring `rate=` after each:
  - `KOKORO_DTYPE=fp32` in `.env` — the `q8` default is smaller, but quantized int8 isn't faster on every CPU, and on some it's markedly slower.
  - `KOKORO_DTYPE=q4` / `q4f16` — smaller still; quality drops, so listen before keeping it.
  - Shorten what's being read: fewer `maxItems` for news, and a tighter `introEverySongs`.

  The timeout scales with script length (`SYNTH_BASE_MS` / `SYNTH_PER_CHAR_MS` in `server/ai/kokoroVoice.js`) and its clock starts when synthesis actually begins, so queueing behind another clip costs a segment nothing.
